		...
		columns : [ [
				...
				},
				{
					...
					formatter : function(value, row, index) {
						...
						var condition;
						if (row.status == 0)
							condition = '<a href="javascript:void(0)" onclick="onSale(this)">上架</a>&emsp;';
						else
							condition = '<a href="javascript:void(0)" onclick="withdraw(this);">下架</a>&emsp;';

						var del = '<a href="javascript:void(0)" onclick="deleteBook(this)">删除</a>';
						return look + update + condition + del;
					}
				} ] ],
...
function look(that) {
	...
	var lookBookContent = '<iframe scrolling="auto" frameborder="0"  src="BookServlet?tp=look&bid='
...
function withdraw(that) {
	$.messager.confirm('确认', '确定下架该书籍吗?', function(r) {
		if (r) {
			var bid = getActionBid(that);
			// ajax基于get方式的请求
			$.get("BookServlet", {
 ...
 * @returns
 */
function onSale(that) {
	$.messager.confirm('确认', '确定上架吗?', function(r) {
		if (r) {
			var bid = getActionBid(that);
			// ajax基于get方式的请求
			$.get("BookServlet", {
...
function deleteBook(that) {
	$.messager.confirm('确认', '确定删除该书籍吗?', function(r) {
		if (r) {
			var bid = getActionBid(that);
			// ajax基于get方式的请求
			$.get("BookServlet", {
...
function addBook(that) {
	var addBookContent = '<iframe scrolling="auto" frameborder="0"  src="addBook.jsp" style="width:100%;height:98%;"></iframe>';
	$('#update_book_dialog').dialog({
		title : '添加书籍',
		width : 500,
		height : 350,
...
function update(that) {
	var bid = getActionBid(that);
	var updateBookContent = '<iframe scrolling="auto" frameborder="0"  src="BookServlet?tp=update&bid='
			+ bid + '" style="width:100%;height:98%;"></iframe>';
	$('#update_book_dialog').dialog({
		title : '更新书籍',
		width : 500,
...
function batchDele() {
	...
	if (rows.length == 0)
		...
		$.messager.confirm('确认', '确定批量删除' + rows.length + '本书籍吗?', function(r) {
			if (r) {
				var bids = [];
